...
<div class="wrraper">
    <v-row align="center" class="card">
      <h2>Edit movie</h2>
      <v-form @submit.prevent="editMovie" ref="editMovieForm" v-model="valid">
        <v-text-field 
        v-model="title"
        :rules="titleRules"
        label="Title"
...
import { getAllMovies, editMovie } from '../../services/movieService'
...
export default {
    ...
    methods: {
      ...
      ...mapActions('movieService', [getAllMovies, editMovie]),
      async editMovie() {
        try {
        ...
        this.$toast.success('Successfully Edited Movie!');
        } catch (err) {
  ...
  created() {
    ...
    this.posterImg = this.movie.posterImg,
